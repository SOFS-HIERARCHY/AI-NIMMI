<!doctype html>
<?php
$page_id="home";
?>
<html class="no-js" lang="">
    
<head>
        <meta charset="utf-8">
        <meta http-equiv="x-ua-compatible" content="ie=edge">
        <title>Anbu Home-Home </title>
        <meta name="description" content="">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="apple-touch-icon" href="img/apple-touch-icon.png">
        <!-- Place favicon.ico in the root directory -->
        <link rel="stylesheet" href="css/bootstrap.css">
        <link rel="stylesheet" href="css/font-awesome.min.css">
        <link rel="stylesheet" href="css/owl.carousel.css">
        <link rel="stylesheet" href="css/owl.theme.css">
        <link rel="stylesheet" href="css/animate.css">
        <link rel="stylesheet" href="css/normalize.css">
        <link rel="stylesheet" href="css/main.css">
        <link rel="stylesheet" href="css/responsive.css">
        <script src="js/modernizr-2.8.3.min.js"></script>
    </head>
    <body>
        
            
        <header>
          
          <!-- header logo: style can be found in header.less -->
       <?php include_once './includes/head.php'; ?>
        <div class="wrapper row-offcanvas row-offcanvas-left">
            <!-- Left side column. contains the logo and sidebar -->
      
            
     </header>
        
        <div class="container-fluid no-padding">
            <div class="main-slider">
                <div class="item">
                    <div class="main-slider-img position-r">
                        <img src="img/page1.jpg" alt="">
                        <div class="overlay"></div>
                        <div class="main-slider-content">
                            <!-- <h2 class="lora fz-20 white text-bold text-italic" >Save the Children</h2>
                            <h3 class="white text-uppercase mt-25 position-r">Knowledge will bring you the <br>opportunity to make a difference.</h3>
                            <h4 class="lora fz-18 white text-bold text-italic mt-30">Volunteers do not Necessarily have the Time, They Just have the Heart.</h4>
                            <div class="mt-30">
                                <a class="martel fz-14 text-bold" href="#">LEARN MORE <i class="fa fa-caret-right"></i></a>
                            </div> -->
                        </div>
                    </div>
                </div>
                <div class="item">
                    <div class="main-slider-img position-r">
                        <img src="img/s2.jpg" alt="">
                        <div class="overlay"></div>
                        <div class="main-slider-content slide-2 text-center">
                        </div>
                    </div>
                </div>
            </div>
        </div>
        
        <section class="mt-100">
            <div class="container">
                <div class="row">
                    <div class="col-md-6 col-sm-6 overcoming">
                                  <h4 class="d-black lh-33 h-sep">Welcome To Anbu Illam</h4>
                          <p class="lh-28 gray-777 fz-15 mt-30">Anbu Illam Trust, Home for the Aged was conceived with the purpose of serving aged peoples in the sunset years of their lives. This was visualized as a residence with a homely atmosphere so that the Senior Citizens would not feel uprooted from the surroundings to which they were accustomed, and to fill their remaining years with much-needed comfort, care and attention. With these objectives in mind, Anbu Illam Trust Home for the Aged Trust started their Home for aged peoples in May 2001, at J.V Farms , Chittrapalayam, Samudram(po), Salem(DT)-636306.</p>
                          <p class="lh-28 gray-777 fz-15 mt-30">'ANBU ILLAM' trust is made on this 30th day of May 2001 between Thiru J. VENKATACHALAM, S/O S.A. Jeyeramalingam aged about 61 years residing at Samudram (Post), Chinnapampatti (Via), Idappadi (Taluk), Salem (District), Tamilnadu here in after called 'Author of the Trust' of the one part</p>
                          <p class="fz-15 gray-777 lh-28 mt-15">Thirumathi V. PADMAJOTHI aged about 57 years W/O J. Venkatachalam, Samudram (Post), Chinnapampatti (Via), Idappadi (Taluk), Salem (District), Tamilnadu representing the first trustees of the second part.</p>
                    </div>
                    <div class="container">
              <img class="img-responsive pull-right t-contact-img" src="img/home.jpg" alt="">
              </div>
                    <!-- <div class="col-md-6 col-sm-6">
                        <div class="count-down">
                            <div class="count-header clearfix">
                                <div class="pull-left">
                                    <h4 class="green-6f text-uppercase mb-15 text-semi-bold">Up Comming Events</h4>
                                    <span class="karla gray-777 fz-14"><i class="fa fa-clock-o"></i> Nov - 14 - 2015    <span class="three-pm">3.00 pm</span></span>
                                </div>
                                <img class="pull-right mt-10" src="img/calendar.png" alt="">
                            </div>

                            <div id="clock"></div>
                            <div class="btns mt-50">
                                <a class="text-uppercase martel fz-14 btn-prime tri-b" href="#">See all our events</a>
                                <a class="text-uppercase martel fz-12" href="#">Join Now</a>
                            </div>
                        </div>
                    </div> -->
                </div>
            </div>
            
        </section>
        
        <section class="mt-150 mb-150">
        <div class="container">
          <div class="row">
              <div class="col-md-12 text-center">
                  <h3 class="text-uppercase black h-sep">At Anbu Illam, we provide high-quality, person-centered care that enables residents to live meaningful lives.</h3>
                  <p class="fz-16 gray-666 mt-50"></p>
              </div>
          </div>

          <div class="row">
            <div class="col-md-4 r-event mt-50">              
              <h4 class="mt-40 fz-15 text-semi-bold d-black">Daily assistance</h4>
              <p class="lh-26 mt-20">Our trained team of care givers aids residents with their activities of daily living (e.g., walking, grooming and assistance with meals.)</p>
               </div>
            <div class="col-md-4 r-event mt-50">              
              <h4 class="mt-40 fz-15 text-semi-bold d-black">24 X 7 care</h4>
             <p class="lh-26 mt-20">Day and night shifts as well as a live-in facility manager ensures that care givers are available around the clock to manage emergencies.</p>
              </div>
              <div class="col-md-4 r-event mt-50">              
              <h4 class="mt-40 fz-15 text-semi-bold d-black">Nursing Care</h4>
              <p class="lh-26 mt-20">Our facility nurses provide basic nursing care for those who need it including Ryle's tube feeding, bed sore management, catheterization, cannulization, wound dressing etc.</p>
              </div>
              </div>
              <div class="row">
            <div class="col-md-4 r-event mt-50">              
              <h4 class="mt-40 fz-15 text-semi-bold d-black">Social engagement</h4>
              <p class="lh-26 mt-20">We provide a supportive atmosphere, frequent one-to-one attention, and a calendar full of activities to ensure each resident is socially engaged.</p>
              </div>
           <div class="col-md-4 r-event mt-50">              
              <h4 class="mt-40 fz-15 text-semi-bold d-black">Nutritious meals</h4>
              <p class="lh-26 mt-20">We serve nutritious and delicious Indian meals, snacks and tea. We also provide regular housekeeping and laundry services.</p>
              </div>
            <div class="col-md-4 r-event mt-50">              
              <h4 class="mt-40 fz-15 text-semi-bold d-black">Dementia Care</h4>
              <p class="lh-26 mt-20">We provide a specialized dementia care program that focuses on emotional well being and symptom management through non pharmacological approaches.</p>
              </div>              
          </div>
           </div>
        </div>
      </section>
        
        <section class="recent-causes gray-f9f9-bg">
            <div class="container">
                <div class="row">
                    <div class="col-md-12 text-center">
                        <h3 class="text-uppercase black h-sep">Recent <span class="text-ultra-bold">Causes</span> </h3>
                        <p class="fz-16 gray-666 mt-50">Our charity helps those people who have no hope</p>
                    </div>
                </div>
                <div class="row mt-70">
                    <div class="col-md-12">
                        <div class="causes">
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/1.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 4500 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">73%</span>
                                    </div>
                                    </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Krishna Raj</h5>
                                    <p class="lh-22 mt-10">Age : 73</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/2.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 5000 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">78%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Periyathambi</h5>
                                    <p class="lh-22 mt-10">Age : 70</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/3.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 4800 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">67%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Sulochana</h5>
                                    <p class="lh-22 mt-10">Age : 60</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/4.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 4000 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">65%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Aarayi</h5>
                                    <p class="lh-22 mt-10">Age : 75</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/5.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 4200 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">76%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Venketasan</h5>
                                    <p class="lh-22 mt-10">Age : 66</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/11.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 4200 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">67%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Bala Subramanian</h5>
                                    <p class="lh-22 mt-10">Age : 68</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/12.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 4600 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">68%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                   <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Pushpa</h5>
                                    <p class="lh-22 mt-10">Age : 65</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/13.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 5100 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">71%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Chitra</h5>
                                    <p class="lh-22 mt-10">Age : 68</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/14.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 5400 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">74%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Lakshmi</h5>
                                    <p class="lh-22 mt-10">Age : 68</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/15.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 4300 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">68%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Saradha</h5>
                                    <p class="lh-22 mt-10">Age : 68</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/16.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 5850 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">77%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                   <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Angappan</h5>
                                    <p class="lh-22 mt-10">Age : 56</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                            <div class="item">
                                <div class="causes-img text-center">
                                    <img src="img/17.jpg" alt="" class="img-responsive">
                                    <div class="badge-c">
                                        <span class="martel text-bold fz-13 white lh-20">Rs. 4700 <br>Monthly<br>Target</span>
                                        <span class="b-round white martel fz-14 text-semi-bold">70%</span>
                                    </div>
                                </div>
                                <div class="cause-content text-center">
                                    <!-- <span class="ubuntu fz-14 gray-777 display-block">Donation: <span class="green-5c">53,458</span> / $87,609 </span> -->
                                    <h5 class="martel text-semi-bold d-black mt-30">Name : Kaliyappan</h5>
                                    <p class="lh-22 mt-10">Age : 80</p>
                                    <a class="fz-14 mt-20 btn-black" href="donate.php">Donate Now</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section class="trust-o">
            <div class="container">
                <div class="row">
                    <div class="col-md-6">
                        <div class="charity-video">
                            <iframe src="https://player.vimeo.com/video/138598929?title=0&amp;byline=0&amp;portrait=0" width="500" height="281" allowfullscreen></iframe>
                        </div>
                    </div>
               <div class="col-md-6">
                        <div class="trust-us">
                            <h2 class="white text-bold h-sep">Who we are ? Organization You Can Trust</h2>
                            <h5 class="karala white text-bold mt-50">We Help 70,0000 Childs, turn tears to smiles. </h5>
                            <p class="white mt-15">Aliquip ex ea cmodo consequat. Duis aute irure dolor in repr ehenderit sachanathaainas voluptate velit esse cillum...</p>
                        </div>
                             </div>
                </div>
            </div>
        </section>

                
        <section class="our-help mt-150">
            <div class="container">
                <div class="row">
                    <div class="col-md-12 text-center">
                        <h3 class=" white h-sep">How Can   <span class="text-ultra-bold">We Help ? </span> </h3>
                  <p class="fz-16 white mt-50">Give a Helping Hand to those who need it!</p>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-3 col-sm-6 mt-70">
                        <div id="canvas-holder" class="clearfix">
                            <canvas id="chart-area" class="pull-left" ></canvas>
                            <div class="round-bar-status pull-left text-center">
                                <h2 class="white text-semi-bold">70%</h2>
                                <h5 class="fz-13 text-semi-bold white text-uppercase mt-10">Medicine</h5>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6 mt-70">
                        <div id="canvas-holder1" class="clearfix">
                            <canvas id="chart-area1" class="pull-left" ></canvas>
                            <div class="round-bar-status pull-left text-center">
                                <h2 class="white text-semi-bold">55%</h2>
                                <h5 class="fz-13 text-semi-bold white text-uppercase mt-10">Food</h5>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6 mt-70">
                        <div id="canvas-holder2" class="clearfix">
                            <canvas id="chart-area2" class="pull-left" ></canvas>
                            <div class="round-bar-status pull-left text-center">
                                <h2 class="white text-semi-bold">40%</h2>
                                <h5 class="fz-13 text-semi-bold white text-uppercase mt-10">Donators</h5>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6 mt-70">
                        <div id="canvas-holder3" class="clearfix">
                            <canvas id="chart-area3" class="pull-left"  ></canvas>
                            <div class="round-bar-status pull-left text-center">
                                <h2 class="white text-semi-bold">60%</h2>
                                <h5 class="fz-13 text-semi-bold white text-uppercase mt-10">Event Program</h5>
                            </div>
                        </div>
                    </div>
                    
                </div>
            </div>
        </section>
         <footer>
          <?php include_once './includes/nav.php'; ?>
            <!-- Right side column. Contains the navbar and content of the page -->
            
        </footer>
        <script src="../../../ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
        <script>window.jQuery || document.write('<script src="js/jquery-1.11.3.min.js"><\/script>')</script>
        <script src="js/bootstrap.min.js"></script>
        <script src="js/owl.carousel.js"></script>
        <script src="js/jquery.countdown.min.js"></script>
        <script src="js/jquery.mixitup.js"></script>
        <script src="js/Chart.min.js"></script>
        <script src="js/custom-chart.js"></script>
        <script src="js/main.js"></script>
    </body>

</html>
